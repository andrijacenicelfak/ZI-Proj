...
namespace Algorithms
{
    ...
    public partial class TigerHash : HashAlgorithm
    {
        private ulong[] Accumulator, x;

        protected int BlockSizeInput;
        protected long ProcessedBytes;
        private byte[] PartialBlockBuffer;
        private int PartialBlockFill;

        public TigerHash() : base()
        {
            this.BlockSizeInput = 64;
            this.HashSizeValue = 192;
            PartialBlockBuffer = new byte[BlockSize];
            Initialize();
        ...
        {
            get { return BlockSizeInput; }
        }
        public int BufferFill
        {
            get { return PartialBlockFill; }
        }

        protected override void HashCore(byte[] array, int startIndex, int blockSize)
        {
            int i;

            if (BufferFill > 0)
            {
                if (blockSize + BufferFill < BlockSize)
                {
                    Array.Copy(array, startIndex, PartialBlockBuffer, BufferFill, blockSize);
                    PartialBlockFill += blockSize;
                    return;
                }
                else
                {
                    i = BlockSize - BufferFill;
                    Array.Copy(array, startIndex, PartialBlockBuffer, BufferFill, i);
                    ProcessBlock(PartialBlockBuffer, 0, 1); ProcessedBytes += BlockSize;
                    PartialBlockFill = 0; startIndex += i; blockSize -= i;
                }
            }

            if (blockSize >= BlockSize)
            {
                ProcessBlock(array, startIndex, blockSize / BlockSize);
                ProcessedBytes += blockSize - blockSize % BlockSize;
            }
            int bytesLeft = blockSize % BlockSize;
            if (bytesLeft != 0)
            {
                Array.Copy(array, ((blockSize - bytesLeft) + startIndex), PartialBlockBuffer, 0, bytesLeft);
                PartialBlockFill = bytesLeft;
            }
        }

        protected override byte[] HashFinal()
        {
            byte[] inputBuffer = PartialBlockBuffer;
            int inputOffset = 0;

            int paddingSize;

            paddingSize = (int)(((ulong)PartialBlockFill + (ulong)ProcessedBytes) % (ulong)BlockSize);
            paddingSize = (BlockSize - 8) - paddingSize;
            if (paddingSize < 1) { paddingSize += BlockSize; }

            if (inputOffset > 0 && PartialBlockFill > 0) Array.Copy(inputBuffer, inputOffset, inputBuffer, 0, PartialBlockFill);
            inputOffset = 0;

            Array.Clear(inputBuffer, PartialBlockFill, BlockSize - PartialBlockFill);
            inputBuffer[PartialBlockFill] = 0x01;
            ulong msg_bit_length = ((ulong)ProcessedBytes + (ulong)PartialBlockFill) << 3;

            if (PartialBlockFill + 8 >= BlockSize)
            {
                if (inputBuffer.Length < 2 * BlockSize) Array.Resize(ref inputBuffer, 2 * BlockSize);
                ProcessBlock(inputBuffer, inputOffset, 1);
                inputOffset += BlockSize; PartialBlockFill -= BlockSize;
            }

            for (PartialBlockFill = inputOffset + BlockSize - sizeof(ulong); msg_bit_length != 0;
                    inputBuffer[PartialBlockFill] = (byte)msg_bit_length, msg_bit_length >>= 8, ++PartialBlockFill) ;
            ProcessBlock(inputBuffer, inputOffset, 1);


            HashValue = new byte[HashSizeValue / 8];
            Buffer.BlockCopy(Accumulator, 0, HashValue, 0, 3 * sizeof(ulong));
            return HashValue;
        }
